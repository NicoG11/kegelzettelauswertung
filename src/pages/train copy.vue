...
const processImage = async img => {
    ...
    const columnX = findColumnX(lines, 'H', 'EI');
    if (columnX !== null) {
...
const findColumnX = (lines, columnName, columnName2) => {
    for (const line of lines) {
        for (const word of line.words) {
            console.log(word.text.trim().toUpperCase());
            if (word.text.trim().toUpperCase() === columnName || word.text.trim().toUpperCase() === columnName2) {
                return word.bbox.x0;
            }
        }
    }
...
watch(extractedImages, () => {
    ...
    console.log(extractedImages.value.length);

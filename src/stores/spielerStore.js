...
export const useSpielerStore = defineStore("spielerStore", {
	...
	actions: {
		...
		getFineFromOtherPlayers(
			...
			if (withCurrency) {
				return `${germanCurrencyFormat(sum)} ${withMax && sum >= MaxToPay ? " - Höchstbetrag" : ""}`;
			}
			return sum;
		},
		...
		getGesamtToPay(player, withCurrency = true) {
			...
			if (withCurrency) {
				return `${germanCurrencyFormat(sum)} ${sum >= MaxToPay ? " - Höchstbetrag" : ""}`;
			}
		...
		groesteStrafe() {
			...
			this.spielerListe.forEach((spieler) => {
				spielerArray.push({
					spieler,
					number: this.getGesamtToPay(spieler, false),
				});
			});
			...
			if (
				spielerArray[0].number > 0 &&
				spielerArray[0].number === spielerArray[1].number
			) {
				...
				if (playerSum2 < playerSum) {
				...
				} else if (playerSum2 > 0 && playerSum2 === playerSum) {
					...
					if (playerAbrSum2 < playerAbrSum) {
						...
						const playerFehlwurfSum2 = this.fehlwurfSum(
							spielerArray[1].spieler,
						);
						if (playerFehlwurfSum2 < playerFehlwurfSum) {
							...
							const playerVolleSum = this.getPlayerVolleSum(
								spielerArray[0].spieler,
							);
							const playerVolleSum2 = this.getPlayerVolleSum(
								spielerArray[1].spieler,
							);
							if (playerVolleSum2 < playerVolleSum) {
								groesteStrafe.sum = germanCurrencyFormat(
									spielerArray[1].number,
								);
								groesteStrafe.number = spielerArray[1].number;
								groesteStrafe.player = spielerArray[1].spieler;
							}
		...
		letzterPlatz() {
			...
			this.spielerListe.forEach((spieler) => {
				number = 0;
				for (let lane = 1; lane <= 4; lane++) {
					number += spieler.bahnen[lane].reduce(
						(a, b) => Number(a) + Number(b),
						0,
					);
				}
				if (
					(number > 0 && number < letzerPlatz.number) ||
					letzerPlatz.number === 0
				) {
					letzerPlatz.number = number;
					letzerPlatz.player = spieler;
				} else if (number > 0 && number === letzerPlatz.number) {
					//check wer hat mehr abgeraeumt
					...
					if (playerAbrSum < letzerPlatzAbrSum) {
						...
						if (playerFehlwurfSum < letzerPlatzFehlwurfSum) {
							...
							const letzerPlatzVolleSum = this.getPlayerVolleSum(
								letzerPlatz.player,
							);
		...
		meisteRatten() {
			...
			this.spielerListe.forEach((spieler) => {
				...
				if (count > 0 && count > meisteRatten.number) {
					meisteRatten.number = count;
					meisteRatten.player = spieler;
				} else if (count > 0 && count === meisteRatten.number) {
					//check wer hat mehr abgeraeumt
					const playerAbrSum = this.getPlayerAbrSum(spieler);
		...
		wenigsteRaumer() {
			...
			this.spielerListe.forEach((spieler) => {
				...
				if (
					(sum > 0 &&
						sum < wenigsteRaumer.number &&
						wenigsteRaumer.number > 0) ||
					wenigsteRaumer.number === 0
				) {
					wenigsteRaumer.number = sum;
					wenigsteRaumer.player = spieler;
				} else if (sum > 0 && sum === wenigsteRaumer.number) {
					//check wer hat mehr gespielt
					const playerSum = this.getPlayerSum(spieler);
					...
					if (playerSum < wenigsteRaumerSum) {
						...
						const playerFehlwurfSum = this.fehlwurfSum(spieler);
						const wenigsteRaumerFehlwurfSum = this.fehlwurfSum(
							wenigsteRaumer.player,
						);
						if (playerFehlwurfSum < wenigsteRaumerFehlwurfSum) {
							...
							//check wer hat mehr volle
							const playerVolleSum = this.getPlayerVolleSum(spieler);
							const wenigsteRaumerVolleSum = this.getPlayerVolleSum(
								wenigsteRaumer.player,
							);
		...
		schlechtesteBahn() {
			...
			this.spielerListe.forEach((spieler) => {
				for (let lane = 1; lane <= 4; lane++) {
					count = 0;
					count = spieler.bahnen[lane].reduce(
						(a, b) => Number(a) + Number(b),
						0,
					);
					...
					if (
						(count > 0 &&
							count < schlechtesteBahn.number &&
							schlechtesteBahn.number > 0) ||
						schlechtesteBahn.number === 0
					) {
						schlechtesteBahn.number = count;
						schlechtesteBahn.player = spieler;
					} else if (count > 0 && count === schlechtesteBahn.number) {
						//check wer hat mehr gespielt
						const playerSum = this.getPlayerSum(spieler);
						const schlechtesteBahnSum = this.getPlayerSum(
							schlechtesteBahn.player,
						);
						if (playerSum < schlechtesteBahnSum) {
							...
							const playerAbrSum = this.getPlayerAbrSum(spieler);
							const schlechtesteBahnAbrSum = this.getPlayerAbrSum(
								schlechtesteBahn.player,
							);
		...
		zweiterPlatz() {
			...
			if (
				spielerErgebnisArray[0]?.number > 0 &&
				spielerErgebnisArray[0]?.number === spielerErgebnisArray[1]?.number
			) {
				const playerAbrSum2 = this.getPlayerAbrSum(
					spielerErgebnisArray[1].spieler,
				);
				const playerAbrSum = this.getPlayerAbrSum(
					spielerErgebnisArray[0].spieler,
				);
				if (playerAbrSum < playerAbrSum2) {
					...
					//check wer hat weniger fehlwuerfe
					const playerFehlwurfSum = this.fehlwurfSum(
						spielerErgebnisArray[0].spieler,
					);
					const playerFehlwurfSum2 = this.fehlwurfSum(
						spielerErgebnisArray[1].spieler,
				...
				spielerErgebnisArray[2].number > 0 &&
				spielerErgebnisArray[1].number === spielerErgebnisArray[2].number
			) {
				...
				const playerAbrSum2 = this.getPlayerAbrSum(
					spielerErgebnisArray[2].spieler,
				);
				if (playerAbrSum < playerAbrSum2) {
		...
		meiste9er() {
			...
			this.spielerListe.forEach((spieler) => {
				...
				if (count > meiste9er.number) {
				...
				} else if (count > 0 && count === meiste9er.number) {
					//check wer hat mehr gespielt
					const playerSum = this.getPlayerSum(spieler);
					...
					if (playerSum > meiste9erSum) {
						...
						if (playerAbrSum > meiste9erAbrSum) {
							meiste9er.number = count;
							meiste9er.player = spieler;
						} else if (playerAbrSum === meiste9erAbrSum) {
							//check wer hat weniger fehlwuerfe
							const playerFehlwurfSum = this.fehlwurfSum(spieler);
							const meiste9erFehlwurfSum = this.fehlwurfSum(meiste9er.player);
							if (playerFehlwurfSum < meiste9erFehlwurfSum) {
								meiste9er.number = count;
								meiste9er.player = spieler;
							} else if (playerFehlwurfSum === meiste9erFehlwurfSum) {
								//check wer hat mehr volle
								const playerVolleSum = this.getPlayerVolleSum(spieler);
								const meiste9erVolleSum = this.getPlayerVolleSum(
									meiste9er.player,
								);
								if (playerVolleSum > meiste9erVolleSum) {
									meiste9er.number = count;
									meiste9er.player = spieler;
								}
							}
		...
		besterSpieler() {
			...
			this.spielerListe.forEach((spieler) => {
				...
				if (number > besterSpieler.number) {
				...
				} else if (number > 0 && number === besterSpieler.number) {
					//check wer hat mehr abgeräumt
					const playerAbrSum = this.getPlayerAbrSum(spieler);
					const besterSpielerAbrSum = this.getPlayerAbrSum(
					...
					if (playerAbrSum > besterSpielerAbrSum) {
						...
						//check wer hat weniger fehlwuerfe
						const playerFehlwurfSum = this.fehlwurfSum(spieler);
						const besterSpielerFehlwurfSum = this.fehlwurfSum(
							besterSpieler.player,
						);
						if (playerFehlwurfSum < besterSpielerFehlwurfSum) {
							...
							//check wer hat mehr volle
							const playerVolleSum = this.getPlayerVolleSum(spieler);
							const besterSpielerVolleSum = this.getPlayerVolleSum(
								besterSpieler.player,
							);
